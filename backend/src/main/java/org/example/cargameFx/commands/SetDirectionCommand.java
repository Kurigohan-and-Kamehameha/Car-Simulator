        ...
        if (State.DRIVE != model.getStates().get(playerId).get()) {
            if (!targetId.equals(model.getPositions().get(playerId).getSnapshot().currentNode().getId())) {
                ...
                if (currentPower < totalWeight) {
                    model.getMessages().get(playerId).setMessage(MessageType.ALERT, "Not enough Power to reach target");
                    dispatcher.dispatch(() -> model.getMessages().get(playerId).notifyObservers());
                    return;
                }
                ...
                dispatcher.dispatch(() -> {
                    ...
                    model.getMessages().get(playerId).notifyObservers();
                });
            }
        }
    }
